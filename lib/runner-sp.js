...
Runner.prototype.run = function() {
  ...
  if (opts.script.config.processor) {
    ...
    if (processor.artilleryRun) {
      processor.artilleryRun(opts.script, self.events);
    }
  ...
  createDispatcher(opts.script, opts.payload, opts.options).then(function(runner) {
    ...
    runner.on('done', (report) => {
      if (opts.script.config.processor.artilleryDone) {
        opts.script.config.processor.artilleryDone();
      }
